...
Position::GenKingMoves(MoveList * mlist, genMovesT genType, bool castling)
{
    ...
    while(*destPtr != NULL_SQUARE)
    {
        ...
        if(addThisMove)
        {
            AddLegalMove(mlist, kingSq, destSq, EMPTY);
        }
        destPtr++;
    }
...
Position::IsLegalMove(simpleMoveT * sm)
{
    ...
    if(sm->promote != EMPTY  &&  mover != PAWN)
    {
        return false;
    }
    ...
    else if(piece_IsSlider(mover))
    {
        ...
        if(mover == ROOK  &&  direction_IsDiagonal(dir))
        {
            return false;
        }
        if(mover == BISHOP  &&  !direction_IsDiagonal(dir))
        {
            return false;
        }
        int delta = direction_Delta(dir);
    ...
    for(x = 1;  x < Count[ToMove]  &&  count < total;  ++x, ++sqPtr)
    {
        ...
        if(pt == mask)
        {
            ...
            if(tryMove == 1)
            {
                ...
                if(tryMove == 1)
                {
                    AddLegalMove(mlist, *sqPtr, target, EMPTY);
                }
            }
        }
    }
...
Position::PrintCompactStr(char * cboard)
{
    ...
    if(ToMove == WHITE)
    {
        if(Board[square_Move(ep, DOWN_LEFT)] != WP  &&
                Board[square_Move(ep, DOWN_RIGHT)] != WP)
        {
            ep = NULL_SQUARE;
        }
    ...
    else
    {
        if(Board[square_Move(ep, UP_LEFT)] != BP  &&
                Board[square_Move(ep, UP_RIGHT)] != BP)
        {
            ep = NULL_SQUARE;
        }
...
inline squareT
randomPawnSquare(void)
{
    return (random32() % 48) + A2;
}
...
Position::Random(const char * material)
{
    ...
    while(1)
    {
        ...
        case 'K':
            if(side == BLACK)
            {
                return ERROR_Corrupt;    // Seen third king!
            }
            side = BLACK;
            break;
        case 'Q':
