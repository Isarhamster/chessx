...
void ChessBrowser::slotContextMenu(const QPoint& pos)
{
    ...
    if(!link.isEmpty())
    {
        ...
        m_VariationUp->setVisible(isVariation && game->model().canMoveVariationUp(m_currentMove));
        m_VariationDown->setVisible(isVariation && game->model().canMoveVariationDown(m_currentMove));
