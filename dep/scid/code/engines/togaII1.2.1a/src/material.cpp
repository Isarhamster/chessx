...
static const int PawnOpening   = 70; // was 100 80
...
static const int QueenOpening  = 975; //1000;
static const int QueenEndgame  = 975; //1000;
...
/*   if (WhiteMinors != BlackMinors) {
		if (WhiteMajors == BlackMajors) {
		  if (WhiteMinors > BlackMinors){
			opening += OpeningExchangePenalty / 2;
			endgame += OpeningExchangePenalty / 2;
		  }
		  else{
			opening -= OpeningExchangePenalty / 2;
			endgame -= OpeningExchangePenalty / 2;
